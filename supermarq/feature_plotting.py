...
def plot_bcg_benchmark(
    data, show=True, savefn=None, spoke_labels=None, legend_loc=(0.75, 0.85), color=None
):
    """
    Create a radar plot of the given benchmarks.
    ...
    #             horizontalalignment='center', verticalalignment='center')
    for d, label in zip(case_data, labels):
        ax.plot(theta, d, label=label, color=color)
        ax.fill(theta, d, alpha=0.25, color=color)
    ax.plot(theta, [1, 0.8, 0.6, 0.4, 0.2, 0.0], alpha=0, color="w")
    ax.set_varlabels(spoke_labels)
